...
public class AttackingUnit : Unit
{
    ...
    public Weapon CurrentWeapon { get { return _weapons[_currentWeaponIndex]; } private set { } }
    //[SerializeField] int _minRange;
    //[SerializeField] int _maxRange;

    //public List<int> DamageList { get => _damageListPerWeapon; set => _damageListPerWeapon = value; }
    ...
    public float CalculateDamage(Unit target, Unit attacker)
    {
        ...
        int baseDamage = _weapons[_currentWeaponIndex].DamageList[(int)target.Type];
    ...
    List<Unit> ScanTargets(AttackingUnit attacker)
    {
        ...
        foreach (var unit in FindObjectsOfType<Unit>())
        {
            ...
            bool IsInRange = (L1Distance(attackerPos, potentialTargetPos) >= attacker.CurrentWeapon.MinRange) && (L1Distance(attackerPos, potentialTargetPos) < attacker.CurrentWeapon.MaxRange);
            bool IsEnemy = attacker.Owner != unit.Owner;
            bool IsDamageable = attacker._weapons[_currentWeaponIndex].DamageList[(int)unit.Type] != 0;
